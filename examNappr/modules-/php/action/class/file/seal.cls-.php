		...
		if($_SESSION['admin_name']!='super' && strstr($_SESSION['exclusive_name'],'sealer')==false){
			if($upper_cat=='.'){
				...
				if($isLeader[0][0]=='leader'){
					...
					$sql="SELECT * FROM apply WHERE category like '".$res['category']."'";
					$sql_count="SELECT count(*) as total_num FROM apply WHERE category like '".$res['category']."'";
				}
			}
			else{
	...
	*/
	function treeData($up_id){
		...
		if($isLeader[0][0]=='leader'){
			...
			$sql="select * from admincat where isDel=0 and exclusive_name!='leader' and upperID =$up_id and id in ($deptId)";
		}
		else{
