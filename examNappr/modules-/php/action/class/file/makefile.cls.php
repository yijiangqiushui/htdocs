...
class makeFile{
	...
	*/
	function save($Info,$admin_id,$attach_name,$file_auto_name){
		...
		$sql2="select count(id) as count from document where file_name='$name' and author=(select realname from admininfo where id=$id) and file_no is null and isInvalid=0 ";
		$res2=$db->Select($sql);
		if($res2!=0){
			echo 'file_name_exit';
		}
	...
	* 查询文件审批列表
	*/
	public function getDateGrid($page,$rows,$upper_cat,$beginDate,$endDate,$file_name,$file_no,$flag1,$file_self,$file_type,$author){
		$db=new DB();
		$page=($page-1)*$rows;
	...
	*/
	function treeData($up_id){
		...
		//判断是否为制文人
		$manager=$db->Select("SELECT exclusive_name FROM admininfo WHERE id=".$_SESSION['admin_id']." limit 0,1");
		if(strpos($manager[0]['exclusive_name'],'drafter')>-1){
			if($up_id==-2){//待审核
