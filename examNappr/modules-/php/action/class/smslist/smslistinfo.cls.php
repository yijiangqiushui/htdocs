	...
	*/
	function infoGrid($upper_cat,$page,$rows,$name,$tel,$company,$add_time,$upid,$admin_id,$listID){
		...
		if($listID==''){
			...
			if($_SESSION['admin_name']=='super'){
				$sql='select a.* from smslistinfo a,smslistcat b where a.admin_id=b.user_id AND a.category=CONCAT(b.upper_cat,b.id,".") and a.category like "'.$upper_cat.'%"';
				$sqlCount='select count(a.id) as "count" from smslistinfo a,smslistcat b where a.admin_id=b.user_id AND a.category=CONCAT(b.upper_cat,b.id,".") and a.category like "'.$upper_cat.'%"';
			}else{
				if(isset($upper_cat)){
					$sql='select a.* from smslistinfo a,smslistcat b where a.admin_id=b.user_id AND a.category=CONCAT(b.upper_cat,b.id,".") and a.category like "'.$upper_cat.'%"';
					$sqlCount='select count(a.id) as "count" from smslistinfo a,smslistcat b where a.admin_id=b.user_id AND a.category=CONCAT(b.upper_cat,b.id,".") and a.category like "'.$upper_cat.'%"';
				}else{
					$sql='select a.* from smslistinfo a,smslistcat b where a.admin_id=b.user_id AND a.category=CONCAT(b.upper_cat,b.id,".")';
					$sqlCount='select count(a.id) as "count" from smslistinfo a,smslistcat b where a.admin_id=b.user_id AND a.category=CONCAT(b.upper_cat,b.id,".")';
				}
				$listIDArr=explode(",",$listID);
				if($listIDArr[0]!=0){
					$sql.=' and b.id in ('.$listID.')';
				}
			}
		}
		...
		if($admin_id==''){
			...
			$k=0;
			if($listIDArr[0]=='0'){
				$sql1="select * from smslistinfo where admin_id=$admin_id order by id desc";
				//$sqlCount1='select count(id) as "count" from smslistinfo where admin_id=$admin_id';
				$arr=$db->Select($sql1);
				//$arr_cou1=$db->Select($sqlCount1);
				//$count=$arr_cou1[0]['count'];
				...
				for($i=0;$i<count($listIDArr);$i++){
					$sql1="select * from smslistinfo where category like '%.".$listIDArr[$i].".' order by id desc";
					//$sqlCount1='select count(id) as "count" from smslistinfo where category like "%.'.$listIDArr[$i].'.';
					$arr=$db->Select($sql1);
					//$arr_cou1=$db->Select($sqlCount1);
					//$count=$arr_cou1[0]['count'];
					for($j=0;$j<count($arr);$j++){
			...
			for($i=0;$i<$rows;$i++){
				...
				$arr2[$i]=array(
					'id'=>$arr1[$i+$page]['id'],
					'category'=>$arr1[$i+$page]['category'],
					'name'=>$arr1[$i+$page]['name'],
					'tel'=>$arr1[$i+$page]['tel'],
					'company'=>$arr1[$i+$page]['company'],
					'job'=>$arr1[$i+$page]['job'],
					'add_time'=>$arr1[$i+$page]['add_time'],
					'admin_id'=>$arr1[$i+$page]['admin_id'],
				);
			}
			if($count===0){
				$json='{"total":0,"rows":[]}';	
	...
	*/	
	function setOthers($ids,$list){
		...
		for($i=0;$i<count($idsArr);$i++){
			...
			$flag=true;
			if($otheridsArr[count($otheridsArr)-1]==''){
				array_pop($otheridsArr);
			}
