...
function loadTree(){
	$("#treeData").tree({
		...
		onBeforeExpand:function(node){
			$(this).tree('options').url='../../../../php/action/page/file/file.act.php?action=treeData&up_id='+node.id+'&contentID='+contentID;
		},
		onClick:function(node){
			...
			if(pridge.exclusive_name.indexOf('drafter')<0){
				if(pridge.exclusive_name.indexOf('leader')<0){
				...
				if(pridge.priviledges.indexOf('file_self')<0){
							...
							$('#infoGrid').datagrid('load',{'upper_cat':upper_cat,'flag1':0,'file_self':1});
				}	
...
function loadPage(){
	...
	$('#infoGrid').datagrid({
		...
		columns:[[
			...
			//{field:'ismake',title:'红头文件',width:35,
			...
			{field:'action',title:'操作',width:80,
				formatter:function(value,row,index){
					...
					//var f=' | '+'<a href="javascript:void(0)" onclick="addFileNo('+row.id+')">生成文件编号</a>';	
					...
					var n=' | '+'<a href="javascript:void(0)" onclick="getword('+row.id+')">导出文件</a>';
					var rn='';
					if(row.isInvalid==1){
						rn='  无效文件';
						...
						if(pridge.priviledges.indexOf('file_edit')>=0 && row.flag2!=1 && row.isOwn==1){
							rn+=a;
						}
						if(pridge.priviledges.indexOf('file_del')>=0 && row.flag2!=1 && row.isOwn==1){
							rn+=b;
						...
						if(pridge.priviledges.indexOf('file_word')>=0){
							rn+=n;
						}
						rn+=n;
					}
...
function findInfo(){
	...
	if(pridge.exclusive_name.indexOf('drafter')<0){
		$('#infoGrid').datagrid('reload',{'file_name':file_name,'beginDate':beginDate,'endDate':endDate,'file_no':file_no,'file_type':file_type,'upper_cat':upper_cat,'author':author});
	}
	else{
		if(pridge.priviledges.indexOf('file_self')<0){
			...
			$('#infoGrid').datagrid('reload',{'file_name':file_name,'beginDate':beginDate,'endDate':endDate,'file_no':file_no,'file_type':file_type,'author':author,'flag1':0,'file_self':1});
		}	
							
	}

	//$('#infoGrid').datagrid('reload',{'file_name':file_name,'beginDate':beginDate,'endDate':endDate,'file_no':file_no,'file_type':file_type,'upper_cat':upper_cat});
	$('#queDlg').dialog('close');
}
		...
		if(data!=1){
			...
			if(pridge.exclusive_name.indexOf('drafter')<0){
				...
				if(pridge.priviledges.indexOf('file_self')<0){
					...
					$('#infoGrid').datagrid('reload',{'flag1':0,'file_self':1});
				}	
		...
		columns:[[
			...
			{field:'action',title:'操作',align:'center',
				formatter:function(value,row,index){
					...
					if(row.ismake!=1){
						if(pridge.priviledges.indexOf('maker')>=0){
							if(pridge.priviledges.indexOf('drafter')>=0){
								rn+=a+b;
							}else{
								if(row.isopen==1){
									rn+=b;
