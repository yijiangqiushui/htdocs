	...
	$.get('../../../../php/action/page/jdgPge.act.php',function(data){
		...
		if(data=='super'){
			...
			if(data.indexOf('sms_query')<0){
				...
				if(pridge.priviledges.indexOf('sms_report')<0){
...
function getCommunicate(){
	...
	$.post('../../../../php/action/page/admin/admininfo.act.php?action=getOthersList',function(data){
		...
		for(var i=0;i<data.length;i++){
			html_str+='<div class="otherList'+i+'"><a href="#" class="otherSmsList'+i+'" onclick="getOtherList('+i+','+data[i].id+',\''+data[i].listID+'\')">'+data[i].realname+'</a><ul id="otherTreeData'+i+'" style="display:none;"></ul></div>';
		}
...
function getOtherList(index,otherID,listID){
	otherListID=listID;
