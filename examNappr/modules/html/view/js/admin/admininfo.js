...
function init(){
	...
	$.get('../../../../php/action/page/jdgPge.act.php',function(data){     //data里面显示的是用户的权限，如果当前的是super则是一种操作，如果是普通用户的话，则会返回当前用户的权限信息
		pridge=data;
		admin_name=data;
	...
	if(exclusive_name=='leader'){
      ...
      <td colspan="3"><input id="managerMoreBm" class="easyui-combox" style="width:420px;" /></td>'
		...
		if($(window).height()<595){
			$('#newdlg').css({'height':'450px','overflow-y':'auto'});
			$('.window').css({'top':'0'});
		}
...
function editAdmin(id){
	...
	$.get('../../../../php/action/page/admin/admininfo.act.php?action=getAdmin&id='+id,function(data){
		...
		if(res.exclusive_name=='leader'){
			...
			//$('.privileges').css('display','none');
			if($(window).height()<595){
				$('#edtdlg').css({'height':'450px','overflow-y':'auto'});
				$('.window').css({'top':'0'});
			}
...
function check_isAll(flag,str){
	...
	if(str=='file'){
		...
		if(sealer_flag){
			$('.sealer'+flag)[0].checked=true;
		}else{
			$('.sealer'+flag)[0].checked=false;
		}
	}else{
		for(var i=0;i<len;i++){
			if($('.'+str+'_role'+flag)[i].checked == true){
