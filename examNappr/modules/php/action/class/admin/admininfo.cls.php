...
class ADMININFO{
	...
	function queryAdmin($isDel,$page,$rows,$upCatory,$usr,$isForbidden){
		$page=($page-1)*$rows;
		$sql='select * from admininfo where isDel='.$isDel.' and id!=1';    //这个是排除了超级管理员的信息，选取了普通管理员的信息
		$sqlCount='select count(id) as "count" from admininfo where isDel='.$isDel.' and id!=1';   //查看admininfo里面有多少有效的管理员
		if($_SESSION['admin_id']!=1){    //如果当前的用户不是超级管理员
			$priviledgesArr=explode(',',$_SESSION['priviledges']);   //则将其权限信息根据,进行分割，得到数组
			$admin_arr=explode('_',$priviledgesArr[2]);    // 然后再根据第二个的权限信息，分割，从而得到一个新的数组
			if($admin_arr[1]>0){                //如果数组中第一个的值大于0，则执行相应的过程
				$str='"%.'.$admin_arr[1].'.%"';   //
	...
	/*
	 * 这个是模仿queryAdmin进行做的，这个函数就是用来显示里面管理人员的 
	 */
	function queryadmin(){
