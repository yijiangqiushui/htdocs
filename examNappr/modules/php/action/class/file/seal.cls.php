...
class Apply{
	...
	* 获取印章申请表格数据
	*/
	public function getDateGrid($page,$rows,$upper_cat,$beginDate,$endDate,$person,$reason,$file_no,$file_content,$datacat_id){
		$page=($page-1)*$rows;
		...
		if($isLeader[0][0]=='leader'){//领导
			if($datacat_id==-1){
				...
				if($upper_cat=='.-2.'){
					...
					if($result[0][0]!=''){
						...
						$arrId=explode(',',$result[0][0]);
						for($i=0;$i<count($arrId);$i++){
							$sql.=" category like '$arrId[$i]%' ";
							$sql_count.=" category like '$arrId[$i]%' ";
							if($i<(count($arrId)-1)){
								$sql.="or";
								$sql_count.="or";
							}else{
			...
			if($result_isManager[0]['isManager']==1&&strstr($_SESSION['exclusive_name'],'sealer')==false){//科长
				if($datacat_id==-3){
					$sql="SELECT * FROM apply WHERE category like '".$result_upper_cat[0]['category']."%' and user='".$_SESSION['realname']."'";
					$sql_count="SELECT count(*) as total_num FROM apply WHERE category like '".$result_upper_cat[0]['category']."%' and user='".$_SESSION['realname']."'";
				}else if($datacat_id==-4){
					$sql="SELECT * FROM apply WHERE category like '".$result_upper_cat[0]['category']."%' and user!='".$_SESSION['realname']."'";
					$sql_count="SELECT count(*) as total_num FROM apply WHERE category like '".$result_upper_cat[0]['category']."%' and user!='".$_SESSION['realname']."'";
				}else{
				...
				if(strstr($_SESSION['exclusive_name'],'sealer')==true){//印章管理员
					if($datacat_id==-5){
						$sql="SELECT * FROM apply WHERE category like '".$result_upper_cat[0]['category']."%' and user='".$_SESSION['realname']."'";
						$sql_count="SELECT count(*) as total_num FROM apply WHERE category like '".$result_upper_cat[0]['category']."%' and user='".$_SESSION['realname']."'";
					}else if($datacat_id==-6){
						if($upper_cat=='.-6.'){
							$sql="SELECT * FROM apply WHERE file_no is null and isInvalid=0";
							$sql_count="SELECT count(*) as total_num FROM apply WHERE file_no is null and isInvalid=0";
						}else{
							$sql="SELECT * FROM apply WHERE category like '".$upper_cat."%' and file_no is null and isInvalid=0";
							$sql_count="SELECT count(*) as total_num FROM apply WHERE category like '".$upper_cat."%' and file_no is null and isInvalid=0";
						}
					}else if($datacat_id==-7){
						if($upper_cat=='.-7.'){
							$sql="SELECT * FROM apply WHERE (file_no is not null or isInvalid=1)";
							$sql_count="SELECT count(*) as total_num FROM apply WHERE (file_no is not null or isInvalid=1)";
						}else{
							$sql="SELECT * FROM apply WHERE category like '".$upper_cat."%' and (file_no is not null or isInvalid=1)";
							$sql_count="SELECT count(*) as total_num FROM apply WHERE category like '".$upper_cat."%' and (file_no is not null or isInvalid=1)";
						}
					}else{
						$sql="SELECT * FROM apply WHERE category like '".$upper_cat."%' ";
						$sql_count="SELECT count(*) as total_num FROM apply WHERE category like '".$upper_cat."%' ";
		...
		if($person!=null&&$person!=''){
			$sql.=" and user like '%$person%' ";
			$sql_count.=" and user like '%$person%' ";
		}
	...
	public function getDateGrid_($page,$rows,$upper_cat,$beginDate,$endDate,$reason,$file_no,$file_content){
		$page=($page-1)*$rows;
		$upper_cat = isset($upper_cat) ? $upper_cat : '.';
	...
	*/
	...
	功能：加载树结构
	*/
	function treeData($up_id,$datacat_id){
