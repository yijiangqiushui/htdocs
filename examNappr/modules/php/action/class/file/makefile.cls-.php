...
class makeFile{
	...
	* 查询文件审批列表
	*/
	public function getDateGrid($page,$rows,$upper_cat,$beginDate,$endDate,$file_name,$file_no,$flag1,$file_self,$file_type){
		$db=new DB();
		$page=($page-1)*$rows;
		...
		if($file_self==1){
			$sql="SELECT * FROM document WHERE author=(select usr from admininfo where id=$id)";
			$sql_count="SELECT count(*) as total_num FROM document WHERE author=(select usr from admininfo where id=$id)";
			
			$flagSql="update document set flag1=$flag1 where author=(select usr from admininfo where id=$id)";
		}
		else{
	...
	function treeData($up_id){
		...
		if($isLeader[0][0]=='leader'){
			...
			if($_SESSION['admin_name']=='super' || $temp[0]==0){
				...
				$sql='select * from admincat where isDel=0 and exclusive_name!="leader" and upperID = '.$up_id;
			}else{
				if($up_id!=0){
					//$sql="select * from contentcat where isDel=0 and upper_id =$up_id and id in (".implode(',',$temp).")";
					//$sql="select * from contentcat where isDel=0 and upper_id =$up_id and upper_cat like '%.$tmp.%'";
					//$sql="select * from admincat where isDel=0 and upperID =$up_id and upperCat like '%.$tmp.%'";
					...
					$sql='select * from admincat where isDel=0 and exclusive_name!="leader" and id='.$temp[0];
				}
			}
		}
		$arr=$db->Select($sql);
