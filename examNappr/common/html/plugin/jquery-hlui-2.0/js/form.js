...
$.extend({
	form:function(id){
		...
		if($('#'+id).find('.ui-form-file').length>0){//if file input found
			$('#'+id).attr({'method':'post','enctype':'multipart/form-data','target':'hlui_submit_iframe'});
			is_up_file=1;
		}
	...
	form_validate:function(form_id){
		var is_all_correct=true;
		$('#'+form_id).find('.ui-form-validate').each(function(){
