...
$.extend({
	...
	tree_apply:function(tree_id){
		...
		$.post(d_opt.json_url,function(data){
			...
			$('.ui-tree-node').each(function(i){
				$(this).find('.ui-tree-img').bind('click',function(){
				...
				$(this).bind('click',{'id':i,'upper_cat':$(this).attr('upper_cat'),'cat_id':$(this).attr('cat-id')},function(e){
					$('.ui-tree-node').removeClass('ui-tree-node-selected');
					$(this).removeClass('ui-tree-node-over');
					$(this).addClass('ui-tree-node-selected');
	...
	tree_action:function(id,user_func,iframe_src,iframe_target,upper_cat,cat_id){
		...
		if(user_func=='' && iframe_src==''){
			alert('参数设置错误！');return false;
		}
		else if(iframe_src==''){
