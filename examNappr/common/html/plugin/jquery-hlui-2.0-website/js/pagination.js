...
$.extend({
	pagination_params:function(id){
		...
		return {'page':$('#'+id).attr('current'),'size':$('#'+id).attr('size'),'params':'?page='+$('#'+id).attr('current')+'&size='+$('#'+id).attr('size')};
	},
...
';
		...
		$('#'+config.id).attr('params',config.params);
	...
	pagination_locate:function(id,i){
		...
		if($('#'+id).attr('user-func')!=''){
			//var params='page='+$('#'+id).attr('current')+'&size='+$('#'+id).attr('size');
			//$('#'+id).attr('params',params);
			eval($('#'+id).attr('user-func')+'()');
		}
	}
});