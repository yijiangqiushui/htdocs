...
class ISNOTREFER {
	function queryNotRefer() {
		...
		$sql = 'select * from application where isRefer=0 and org_id=' . $_SESSION ['org_id'] . ' ORDER BY id DESC';
		$sqlCount = 'select count(id) as "count" from  application where isRefer=0 and org_id=' . $_SESSION ['org_id'] . ' ORDER BY id DESC';
