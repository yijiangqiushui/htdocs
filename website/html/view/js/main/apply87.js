...
$(function(){	
	...
	$('#grid').datagrid({
		...
		url:'../../../../php/action/page/apply87.act.php?action=query&id='+id+'&flag='+flag1,
	
		columns:[[				
			{field:'id',title:'id',checkbox:true},
			{field:'name',title:'材料名称',width:80},
			{field:'descrip',title:'材料描述（包括代表性论文发表及被他人引用情况，科普作品的出版及发行情况等）',width:300},
			...
			{field:'action',title:'操作',width:80,align:'center',
				formatter:function(value,row,index){
					...
					var c='<a href="javascript:void(0)" onclick="showAtt('+row.id+')"> 相关文件</a>';
					return a+b+c;
				}
			}										
		]]
...
function showform(fg,b){
	if(fg==0){
		...
		$('#fm').form('clear');
		url='../../../../php/action/page/apply87.act.php?action=add&id='+id;
		flag=0;
	}else{
		$.get( '../../../../php/action/page/apply87.act.php?action=findbyid&id='+fg,function(data){
			
			var res=eval("("+data+")");				
			$('#dlg').dialog('open').dialog('setTitle','修改信息');
			$('#fm').form('load',res);
			url='../../../../php/action/page/apply87.act.php?action=update&id='+fg;
			flag=1;
			dex=b;
   		});		
	}
...
function saveData(){
		if($("input[name='name']").val() == ""){
           $.messager.alert('消息提示','请添加材料名称','info');
	}else{
	$('#fm').form('submit',{
		url:url,
		onSubmit: function(){			
...
function delData(id,index){
	 $.messager.confirm('提示信息','确定删除吗？',function(r){
		 if(r){
			 $.get( '../../../../php/action/page/apply87.act.php?action=delete&id='+id,function(){						
					$('#grid').datagrid('reload');                       
			 });					 
		 }
     }); 
...
function deleteList(){
	...
	if(rows.length==0){
		...
		$.messager.confirm('消息提示','确定删除吗？',function(r){
               if (r){
                      $.get( '../../../../php/action/page/apply87.act.php?action=deletelist&idlist='+list,function(data){				
                           $('#grid').datagrid('reload');
                      });					 
                }
        });					
...
function showAtt(proid){
	...
	$('#att_grid').datagrid({
		...
		url:'../../../../php/action/page/att.act.php?action=query&pro=proof7&pid='+proid,
...
function showAttForm(a){
	if(a==0){
		...
		url='../../../../php/action/page/att.act.php?action=add&pro=proof7&pid='+proofId+'&id='+id;

	}else{
		$.get( '../../../../php/action/page/att.act.php?action=findbyid&pro=proof7&pid='+proofId+'&id='+a,function(data){
			var res=eval("("+data+")");				
			$('#addAtt_dlg').dialog('open').dialog('setTitle','修改信息');
			$('#addAtt_fm').form('load',res);
			url='../../../../php/action/page/att.act.php?action=update&pro=proof7&pid='+proofId+'&id='+a;
   		});	
	}
}
