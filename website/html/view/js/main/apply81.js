...
$(function(){
	...
	$('#grid').datagrid({
		...
		url:'../../../../php/action/page/apply81.act.php?action=query&id='+id+'&flag='+flag1,
...
function showAtt(proid){
	...
	$('#att_grid').datagrid({
		...
		url:'../../../../php/action/page/att.act.php?action=query&pro=proof1&pid='+proid,
...
function showAttForm(a){
	if(a==0){
		...
		url='../../../../php/action/page/att.act.php?action=add&pro=proof1&pid='+proofId+'&id='+id;

	}else{
		$.get( '../../../../php/action/page/att.act.php?action=findbyid&pro=proof1&pid='+proofId+'&id='+a,function(data){
			var res=eval("("+data+")");				
			$('#addAtt_dlg').dialog('open').dialog('setTitle','修改信息');
			$('#addAtt_fm').form('load',res);
			url='../../../../php/action/page/att.act.php?action=update&pro=proof1&pid='+proofId+'&id='+a;
   		});	
	}
}
...
function showform(fg,b){
	if(fg==0){
		...
		$('#fm').form('clear');
		url='../../../../php/action/page/apply81.act.php?action=add&id='+id;
		flag=0;
	}else{
		$.get( '../../../../php/action/page/apply81.act.php?action=findbyid&id='+fg,function(data){
			
			var res=eval("("+data+")");				
			$('#dlg').dialog('open').dialog('setTitle','修改信息');
			$('#fm').form('load',res);
			url='../../../../php/action/page/apply81.act.php?action=update&id='+fg;
			flag=1;
			dex=b;
   		});
...
function saveData(){	
	...
	if($("input[name='name']").val() == ""){
           $.messager.alert('消息提示','请添加授权项目名称','info');
	}else{
	$('#fm').form('submit',{
		url:url,
		onSubmit: function(){			
...
function delData(id,index){
	 $.messager.confirm('提示信息','确定删除吗？',function(r){
		 if(r){
			 $.get( '../../../../php/action/page/apply81.act.php?action=delete&id='+id,function(){						
					$('#grid').datagrid('reload');                        
			 });					 
		 }
     }); 
...
function deleteList(){
	...
	if(rows.length==0){
		...
		$.messager.confirm('消息提示','确定删除吗？',function(r){
               if (r){
                      $.get( '../../../../php/action/page/apply81.act.php?action=deletelist&idlist='+list,function(data){				
                           $('#grid').datagrid('reload');
                      });					 
                }
        });					
