function submit_data(){
		$('#service_thing').form('submit' ,{
			url: '/modules/php/action/page/applycation/apply9.act.php?action=10service_thing',
			success: function(result){
				$.messager.alert('提示','信息保存成功！','info',function(){
				var complete = completetextarea();
				if(complete){
					
					window.parent.save_stage('service_thing');
				}else{
					window.parent.update_stage('service_thing');
				} 
				window.parent.setStep('service_thing',complete);
			});
			}
		});
	}
...
$(function()
	...
	function loadApplyInfo(){
		$.get('/modules/php/action/page/applycation/apply9.act.php?action=10findservice_thing',
		function(result){
			if(result!='0'){
				var res=eval("("+result+")");
				$('#service_thing').form('load',res);
			}
		});
