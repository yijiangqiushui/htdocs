function org_info(){
	$('#other_rule').form('submit',{
		url:'../attach3_patent/attach3_patent.act.php?action=other_rule',
		success:function(result){
			...
			if(complete) {
				...
				window.parent.save_stage('other_rule');
			}else{
				window.parent.update_stage('other_rule');
			}
			window.parent.setStep('other_rule',complete);
			});
		}
	});
}
...
function loadApplyInfo() {
	$.get('../attach3_patent/attach3_patent.act.php?action=findother_rule',
					function(result) {
						if (result != '0') {
							var res = eval("(" + result + ")");
							$('#other_rule').form('load', res);
						}
