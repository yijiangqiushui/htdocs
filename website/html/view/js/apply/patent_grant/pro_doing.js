function org_info(){
	$('#pro_doing').form('submit',{
		url:'../attach3_patent/attach3_patent.act.php?action=pro_doing',
		success:function(result){
			...
			if(complete) {
				...
				window.parent.save_stage('pro_doing');
			}else{
				window.parent.update_stage('pro_doing');
			}
			window.parent.setStep('pro_doing',complete);
			});
		}
	});
}
...
function loadApplyInfo() {
	$.get('../attach3_patent/attach3_patent.act.php?action=findpro_doing',
					function(result) {
						if (result != '0') {
							var res = eval("(" + result + ")");
							$('#pro_doing').form('load', res);
							year();
						}
