function submit_data(){
		$('#service_job').form('submit' ,{
			url: '/modules/php/action/page/applycation/apply9.act.php?action=10service_job',
			success: function(result){
				$.messager.alert('提示','信息保存成功！','info',function(){
				var complete = completetextarea();
				if(complete){
					
					window.parent.save_stage('service_job');
				}else{
					window.parent.update_stage('service_job');
				} 
				window.parent.setStep('service_job',complete);
			});
			}
		});
	}
...
$(function()
	...
	function loadApplyInfo(){
		$.get('/modules/php/action/page/applycation/apply9.act.php?action=10findservice_job',
		function(result){
			if(result!='0'){
				var res=eval("("+result+")");
				$('#service_job').form('load',res);
			}
		});
