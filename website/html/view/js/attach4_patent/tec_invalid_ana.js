function org_info(){
	$('#tec_invalid_ana').form('submit',{
		url:'../../../../../../../modules/php/action/page/applycation/attach4_patent.act.php?action=save_techOrg',
		success:function(result){
			$.messager.alert('提示','信息保存成功！','info',function(){
			var complete = completeInput();
			if(complete) {
				window.parent.save_stage('tec_invalid_ana');
			}else{
				window.parent.update_stage('tec_invalid_ana');
			}
			window.parent.setStep('tec_invalid_ana',complete);
			});
		}
	});
}
...
function loadApplyInfo() {
	$.get('../../../../../../../modules/php/action/page/applycation/attach4_patent.act.php?action=find_techOrg',
					function(result) {
						if (result != '0') {
							var res = eval("(" + result + ")");
							$('#tec_invalid_ana').form('load', res);
						}
					});
