function org_info(){
	$('#market_situation').form('submit',{
		url:'../../../../../../../modules/php/action/page/applycation/attach4_patent.act.php?action=save_market',
		success:function(result){
			$.messager.alert('提示','信息保存成功！','info',function(){
			var complete = completeInput();
			if(complete) {
				window.parent.save_stage('market_situation');
			}else{
				window.parent.update_stage('market_situation');
			}
			window.parent.setStep('market_situation',complete);
			});
		}
	});
}
...
function loadApplyInfo() {
	$.get('../../../../../../../modules/php/action/page/applycation/attach4_patent.act.php?action=find_market',
					function(result) {
						if (result != '0') {
							var res = eval("(" + result + ")");
							$('#market_situation').form('load', res);
						}
					});
