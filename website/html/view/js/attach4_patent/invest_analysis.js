function org_info(){
	$('#invest_analysis').form('submit',{
		url:'../../../../../../../modules/php/action/page/applycation/attach4_patent.act.php?action=save_invest',
		success:function(result){
			$.messager.alert('提示','信息保存成功！','info',function(){
			var complete = completeInput();
			if(complete) {
				window.parent.save_stage('invest_analysis');
			}else{
				window.parent.update_stage('invest_analysis');
			}
			window.parent.setStep('invest_analysis',complete);
			});
		}
	});
}
...
function loadApplyInfo() {
	$.get('../../../../../../../modules/php/action/page/applycation/attach4_patent.act.php?action=find_invest',
					function(result) {
						if (result != '0') {
							var res = eval("(" + result + ")");
							$('#invest_analysis').form('load', res);
						}
					});
