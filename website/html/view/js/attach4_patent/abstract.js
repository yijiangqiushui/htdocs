function org_info(){
	$('#abstract').form('submit',{
		url:'../../../../../../../modules/php/action/page/applycation/attach4_patent.act.php?action=saveAbstract',
		success:function(result){
			$.messager.alert('提示','信息保存成功！','info',function(){
			var complete = completeInput();
			if(complete) {
				window.parent.save_stage('abstract');
			}else{
				window.parent.update_stage('abstract');
			}
			window.parent.setStep('abstract',complete);
			});
		}
	});
}
...
function loadApplyInfo() {
	$.get('../../../../../../../modules/php/action/page/applycation/attach4_patent.act.php?action=find_Abstract',
					function(result) {
						if (result != '0') {
							var res = eval("(" + result + ")");
							$('#abstract').form('load', res);
						}
