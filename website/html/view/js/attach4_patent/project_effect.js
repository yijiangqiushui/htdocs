function org_info(){
	$('#project_effect').form('submit',{
		url:'../../../../../../../modules/php/action/page/applycation/attach4_patent.act.php?action=save_effect',
		success:function(result){
			$.messager.alert('提示','信息保存成功！','info',function(){
			var complete = completeInput();
			if(complete) {
				window.parent.save_stage('project_effect');
			}else{
				window.parent.update_stage('project_effect');
			}
			window.parent.setStep('project_effect',complete);
			});
		}
	});
}
...
function loadApplyInfo() {
	$.get('../../../../../../../modules/php/action/page/applycation/attach4_patent.act.php?action=find_effect',
					function(result) {
						if (result != '0') {
							var res = eval("(" + result + ")");
