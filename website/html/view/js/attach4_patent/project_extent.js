...
function org_info(){
	$('#project_extent').form('submit',{
		url:'../../../../../../../modules/php/action/page/applycation/attach4_patent.act.php?action=save_extent',
		success:function(result){
			$.messager.alert('提示','信息保存成功！','info',function(){
			var complete = completeInput();
			if(complete) {
				window.parent.save_stage('project_extent');
			}else{
				window.parent.update_stage('project_extent');
			}
			window.parent.setStep('project_extent',complete);
			});
		}
	});
}
...
function loadApplyInfo() {
	$.get('../../../../../../../modules/php/action/page/applycation/attach4_patent.act.php?action=find_extent',
					function(result) {
						if (result != '0') {
							var res = eval("(" + result + ")");
							$('#project_extent').form('load', res);
						}
					});
