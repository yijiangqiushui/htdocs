...
$(function(){
	...
	var p4 = $('#datagrid4').datagrid('getPager');  
	$(p4).pagination({  
			pageSize: 5,  
			pageList: [5,10,15],  
			beforePageText: '第', 
	...
	$('#datagrid5').datagrid({
		...
		columns:[[
			{field:'id',title:'id',hidden:true},
			{field:'applicationName',title:'项目名称',width:200},
	...
	$('#datagrid87').datagrid({
		...
		url:'../../../php/action/page/apply87.act.php?action=query&id='+id,
	...
	var p87 = $('#datagrid87').datagrid('getPager');			  
	$(p87).pagination({  
			pageSize: 5,  
			pageList: [5,10,15],  
			beforePageText: '第', 
...
function setParameters(){
	...
	paramArr.forEach(function(elem){
		var elemArr=elem.split('=');
		switch(elemArr[0]){
			case 'id':id=parseInt(elemArr[1]);break;
		}
	});
}