...
function addLine(){
    ...
    newTr +="<th><p align='center'>专利名称</p></th>";
	 ...
	 newTr +="<td><select name='patent_type["+$val+"]' ><option value='发明'>发明</option><option value='实用新型'>实用新型</option><option value='外观设计'>外观设计</option></select></td>"
	 newTr  +="<td rowspan='2'><input type='button'  value='删除'  class='pointer'  onclick='deleteTwoRow(this)'/></td>";
	 newTr  +="</tr>";
	 newTr  +="<tr>";
	newTr  +="<th><p align='center'>专利状态</p></th>";
	 //newTr +="<td><input type='text' name=\"patent_state["+$val+"]\" /></td>";
	newTr +="<td><select name='patent_state["+$val+"]' ><option value='申请'>申请</option><option value='实审'>实审</option><option value='授权有效'>授权有效</option></select></td>";
	 newTr +="</tr>";
