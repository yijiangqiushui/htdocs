function saveCheckOpin(){
	$('#other_condition_form').form('submit',{
		url:'../../../../../modules/php/action/page/projectapp/projectapp.act.php?action=saveOtherCondition',
		success:function(result){
			$.messager.alert('提示','信息保存成功！','info',function(){
				...
				if(complete) {
					//call save_stage
					window.parent.save_stage('other_condition');
				} else{
					window.parent.update_stage('other_condition');
				}
				window.parent.setStep('other_condition',complete);
				});
		}
	});
}
...
$(function()
	...
	function loadApplyInfo(){
		$.get('../../../../../modules/php/action/page/projectapp/projectapp.act.php?action=findOtherCondition',function(result){
			if(result!='0'){
				var res=eval("("+result+")");
				$('#other_condition_form').form('load',res);
			}
		});
...
function resetInfo(){
	$('#other_condition_form').form('reset');
}
