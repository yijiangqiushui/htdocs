function pro_expect(){
	$('#project_expect').form('submit',{
		url:'../../../../../modules/php/action/page/projectapp/projectapp.act.php?action=task_project_expect',
		success:function(result){
			$.messager.alert('提示','信息保存成功！','info',function(){
				...
				if(complete) {
					//call save_stage
					window.parent.save_stage('project_expert_form');
				} else{
					window.parent.update_stage('project_expert_form');
				}
				window.parent.setStep('project_expert_form',complete);
				});
		}
	});
}
...
$(function()
	...
	function loadApplyInfo(){
		$.get('../../../../../modules/php/action/page/projectapp/projectapp.act.php?action=findtask_ProExpert',function(result){
			if(result!='0'){
				var res=eval("("+result+")");
