...
date:2014-05-06
*/
var upid=0,uptext='根节点',upcat,edtid;
...
$(function() {
		...
		$.get('../../../php/action/page/jdgPge.act.php',function(data){
			...
			if(data=='super'){
				...
				if(data.indexOf('contentcat_query')<0){
					$('body').html('<h2>您没有操作权限</h2>');
					
				}else{
					if(data.indexOf('contentcat_del')<0){
							$("a[onclick='delArrCat()']").css('display','none');			
					}
					if(data.indexOf('contentcat_add')<0){
							$("a[onclick='newCat()']").css('display','none');			
					}
					if(data.indexOf('contentcat_disable')<0){
							$("a[iconCls='icon-no']").css('display','none');			
					}
					if(data.indexOf('contentcat_enable')<0){
...
function loadPage(){
	$('#catGrid').datagrid({
		...
		url:'../../../php/action/page/content/contentcat.act.php?action=catGrid',//url调用Action方法
		columns:[[
			...
			{field:'action',title:'操作',width:150,align:'center',
				formatter:function(value,row,index){							
					...
					if(pridge=='super'){
							...
							if(pridge.indexOf('contentcat_edit')>=0){
								rn+=a;
							}
							if(pridge.indexOf('contentcat_del')>=0){
								rn+=b;
							}
						}
