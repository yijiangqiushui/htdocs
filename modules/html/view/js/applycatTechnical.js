...
function loadTree(){
	$('#treeData').tree({
		url:'../../../php/action/page/applycation/applycatTechnical.act.php?action=treeData',
		onBeforeExpand:function(node){
			$(this).tree('options').url='../../../php/action/page/applycation/applycatTechnical.act.php?action=treeData&upper_id='+node.id;
		},
...
function loadPage(){
	$('#catgrid').datagrid({
		...
		url:'../../../php/action/page/applycation/applycatTechnical.act.php?action=gridData',//url调用Action方法
...
function newCat(){
	...
	url = '../../../php/action/page/applycation/applycatTechnical.act.php?action=saveCat';
}
...
function edtCat(id){
	$.get('../../../php/action/page/applycation/applycatTechnical.act.php?action=findCat&id='+id,function(result){
		$('#edtFm').form('clear');
		loadcombotree();
...
function saveEdtCat(){
	$('#edtFm').form('submit',{
		url:'../../../php/action/page/applycation/applycatTechnical.act.php?action=saveEdtCat&id='+edtid,
		success:function(result){
			var node=$("#treeData").tree('find',upper_id);
			result=eval('('+result+')');
...
function delCat(id){
	$.messager.confirm('提示信息','确定删除吗？',function(r){
     	if (r){
      		$.get('../../../php/action/page/applycation/applycatTechnical.act.php?action=delCat&id='+id,function(){
           		$('#catgrid').datagrid('reload');
...
function delCatlist(){
	...
	if(rows.length==0){
		...
		$.messager.confirm('消息提示','确定删除吗？',function(r){
			if (r){
			   $.get('../../../php/action/page/applycation/applycatTechnical.act.php?action=delCatlist&idlist='+list,function(data){
				   $('#catgrid').datagrid('reload');
				   var arr=data.split(',');
...
function disableCat(flag){
	...
	if(rows.length==0){
		...
		$.messager.confirm('消息提示',smessage,function(r){
			if(r){
				$.get('../../../php/action/page/applycation/applycatTechnical.act.php?action=disableCat&idlist='+list+'&flag='+flag,function(data){
					$('#catgrid').datagrid('reload');
				});
			}
		});
