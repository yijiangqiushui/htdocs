...
function column_type_comm(){
	$.get('../../../php/action/page/chart/chart.act.php?action=type_comm',function(data){
		var data=eval('('+data+')');		
		var xline=[];
		var source=[];
...
//按地域 柱状图显示
function column_address_comm(){
	$.get('../../../php/action/page/chart/chart.act.php?action=address_comm',function(data){
		var data=eval('('+data+')');
...
function pie_type_comm(){
	$.get('../../../php/action/page/chart/chart.act.php?action=type_comm',function(data){
		...
		var source='[';		
		for(var i=0;i<data.length;i++){
			source=source+'["'+data[i].type+'",'+parseInt(data[i].count)+'],'
		}		
