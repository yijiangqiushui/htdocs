		...
		$(function() {
			...
			$.get('../../../php/action/page/jdgPge.act.php',function(data){
				...
				if(data=='super'){
					...
					if(data.indexOf('exp_query')<0){
						$('body').html('<h2>您没有操作权限</h2>');
						
					}else{
						if(data.indexOf('exp_del')<0){
								//$('.exp_del').css('display','none');
								$("a[onclick='destroyExpertlist()']").css('display','none');			
						}
						if(data.indexOf('exp_add')<0){
								$("a[onclick='newExpert()']").css('display','none');			
						}
						loadPage();
					}	
				}
		...
		function loadPage(){
			...
			$('#grid').datagrid({
	            ...
	            url:'../../../php/action/page/expert/exp.act.php?action=query',//url调用Action方法
	            loadMsg : '数据装载中......',
	          	...
	          	columns:[[
					...
					{field:'action',title:'操作',width:300,align:'center',
						formatter:function(value,row,index){
							var a='<a href="javascript:void(0)" onclick="editExpert('+row.id+')" class="exp_edit">编辑</a>';
							var b='|'+'<a href="javascript:void(0)" onclick="destroyExpert('+row.id+')" class="exp_del"> 删除</a>';
							var c='|'+'<a href="javascript:void(0)" onclick="showEduInfo('+row.id+')" class="exp_edu"> 教育经历</a>';
							var d='|'+'<a href="javascript:void(0)" onclick="showHorInfo('+row.id+')" class="exp_hor"> 曾获荣誉</a>';
							var e='|'+'<a href="javascript:void(0)" onclick="showComInfo('+row.id+')" class="exp_com"> 指导竞赛情况</a>';
							var rn='';
							if(pridge=='super'){
								rn=a+b+c+d+e;	
							}else{
								if(pridge.indexOf('exp_edit')>=0){
									rn+=a;
								}
								if(pridge.indexOf('exp_del')>=0){
									rn+=b;
								}
								if(pridge.indexOf('exp_edu')>=0){
									rn+=c;
								}
								if(pridge.indexOf('exp_hor')>=0){
									rn+=d;
								}
								if(pridge.indexOf('exp_com')>=0){
									rn+=e;
								}
							}	
							return rn;
			...
			if(($('#search_degree').combobox('getValue')=='')&&($('#search_name').val()=='')){
				$('#show').dialog('close');
				 ...
				 $('#show-form').form('submit',{
					url:'../../../php/action/page/expert/exp.act.php?action=query',
					onSubmit: function(){
        ...
        function newExpert(){
            ...
            $('#fm').form('clear');
			url = '../../../php/action/page/expert/exp.act.php?action=add';
        }
		//修改信息
        function editExpert(a){
			
			$.get( '../../../php/action/page/expert/exp.act.php?action=findbyid&id='+a,function(data){
			
				var res=eval("("+data+")");				
            	$('#dlg').dialog('open').dialog('setTitle','修改信息');
                $('#fm').form('load',res);
				
                url = '../../../php/action/page/expert/exp.act.php?action=update&id='+a;
		...
		//删除
        function destroyExpert(id){
                $.messager.confirm('提示信息','确定删除吗？',function(r){
                    if (r){
                        $.get( '../../../php/action/page/expert/exp.act.php?action=delete&id='+id,function(){						
                           $('#grid').datagrid('reload');                      
                        });					 
                    }
                });         
		...
		//批量删除
		function destroyExpertlist(){
			
			var rows = $("#grid").datagrid("getChecked");
			
			if(rows.length==0){
				$.messager.alert('消息提示','请选择要删除的专家信息','info');	
			}else{
				var list=new Array();
				...
				$.messager.confirm('消息提示','确定删除吗？',function(r){
                    if (r){
                        $.get( '../../../php/action/page/expert/exp.act.php?action=deletelist&idlist='+list,function(data){					
                           $('#grid').datagrid('reload');                
                        });					 
                    }
                });
		...
		function showEduInfo(tid){
	        ...
	        $('#edu_grid').datagrid({
	            ...
	            url:'../../../php/action/page/expert/exp.act.php?action=queryEdu&tid='+tid,//url调用Action方法
	            loadMsg : '数据装载中......',
		...
		function showEduAddForm(){
			...
			$('#edu_form').form('clear');
			url='../../../php/action/page/expert/exp.act.php?action=addEdu&tid='+expertId;
		}
		
		function showEduEdtForm(id){
			
			$.get( '../../../php/action/page/expert/exp.act.php?action=findEduById&id='+id,function(data){
					
				data=eval("("+data+")");				
            	$('#edu_dlg').dialog('open').dialog('setTitle','修改教育经历');
                $('#edu_form').form('load',data);
				
               url = '../../../php/action/page/expert/exp.act.php?action=updateEdu&id='+id+'&tid='+expertId;
		...
		function delEdu(id){
                $.messager.confirm('提示信息','确定删除吗？',function(r){
                    if (r){
                        $.get( '../../../php/action/page/expert/exp.act.php?action=delEdu&id='+id,function(){						
                           $('#edu_grid').datagrid('reload');                      
                        });					 
                    }
                });         
		...
		function showHorInfo(tid){
	        ...
	        $('#hor_grid').datagrid({
	            ...
	            url:'../../../php/action/page/expert/exp.act.php?action=queryHor&tid='+tid,//url调用Action方法
	            loadMsg : '数据装载中......',
		...
		function showHorAddForm(){
			...
			url='../../../php/action/page/expert/exp.act.php?action=addHor&tid='+expertId;
		}
		
		function showHorEdtForm(id){
			
			$.get( '../../../php/action/page/expert/exp.act.php?action=findHorById&id='+id,function(data){
					
				data=eval("("+data+")");				
            	$('#hor_dlg').dialog('open').dialog('setTitle','修改荣誉信息');
                $('#hor_form').form('load',data);
				
               url = '../../../php/action/page/expert/exp.act.php?action=updateHor&id='+id+'&tid='+expertId;
		...
		function delHor(id){
                $.messager.confirm('提示信息','确定删除吗？',function(r){
                    if (r){
                        $.get( '../../../php/action/page/expert/exp.act.php?action=delHor&id='+id,function(){						
                           $('#hor_grid').datagrid('reload');                      
                        });					 
                    }
                });         
	 ...
	 //-----------------------------------指导竞赛情况--------------------------------------
		function showComInfo(tid){
	        ...
	        $('#com_grid').datagrid({
	            ...
	            url:'../../../php/action/page/expert/exp.act.php?action=queryCom&tid='+tid,//url调用Action方法
	            loadMsg : '数据装载中......',
		...
		function showComAddForm(){
			...
			url='../../../php/action/page/expert/exp.act.php?action=addCom&tid='+expertId;
		}
		
		function showComEdtForm(id){
			
			$.get( '../../../php/action/page/expert/exp.act.php?action=findComById&id='+id,function(data){
					
				data=eval("("+data+")");				
            	$('#com_dlg').dialog('open').dialog('setTitle','修改信息');
                $('#com_form').form('load',data);
				
               url = '../../../php/action/page/expert/exp.act.php?action=updateCom&id='+id+'&tid='+expertId;
		...
		function delCom(id){
                $.messager.confirm('提示信息','确定删除吗？',function(r){
                    if (r){
                        $.get( '../../../php/action/page/expert/exp.act.php?action=delCom&id='+id,function(){						
                           $('#com_grid').datagrid('reload');                      
                        });					 
                    }
                });         
