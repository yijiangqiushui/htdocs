...
class APPLY {
	// 附件8 org_info.html入库
	function org_info8($project_id, $project_info, $org_code, $staff_info, $arrData1, $arrData2, $arrData3, $arrData4,$linkmanInfo,$project_id) {
		$db = new DB ();
	...
	function find_org_info8_pdf($org_code, $project_id) {
	    $db = new DB ();
	    ...
	    $appJSON = array (
	        ...
	        'expect_income' => $res4 ['predict_income'],
	        'expect_tax' => $res4 ['predict_tax'],
	...
	// 附件8 org_info.html 获取数据库
	function find_org_info8($org_code, $project_id) {
		$db = new DB ();
		$res1 = $db->GetInfo1 ( $org_code, 'org_info', 'org_code' );
		$res2 = $db->GetInfo1 ( $org_code, 'legal_person', 'org_code' );
		$res3 = $db->GetInfo1 ( $org_code, 'intellectual_property', 'org_code' );
		...
		// 这里主要是选择一个用户必须填写的项作为判断当前是否填满的标示，这里选择了负责人姓名
		if ($result ['legal_name'] != null) {
		    $appJSON ['mark'] = '1';
		} else {
		    $appJSON ['mark'] = '0';
		}
		
		$sql = "SELECT table_status FROM table_status WHERE project_id = '$project_id' and table_name ='北京市通州区科技计划项目实施方案'";
