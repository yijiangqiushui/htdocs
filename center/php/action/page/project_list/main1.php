    ...
    for($i = 1;$i <= $row_n;$i++){
        ...
        if($row_m != 0){
            ...
            if($father->id == $father->father  && $father->apply_status == 1 ){
                $start_time = date("Y-m-d",floatval($father->apply_start));
                $end_time = date("Y-m-d",floatval($father->apply_end));
            ...
            for($j = 1;$j <= $row_m;$j++){
                ...
                if($children->father != $children->id  && $children->apply_status == 1){
                    ...
                    $start_time = date("Y-m-d", floatval($children->apply_start));
                    $end_time = date("Y-m-d",floatval($children->apply_end));
                    echo "<td  align='center'>".$start_time."</td>";
                    echo "<td  align='center'>".$end_time."</td>";
