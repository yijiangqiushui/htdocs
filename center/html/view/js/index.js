...
function submitForm(){
	...
	$.post("../../../php/action/page/index.php",{NTID:NTID,Digest:hashValue,flag:flag,username:$.trim($('#username').val()),password:$.trim($('#password').val()),validateCode:$.trim($('#validateCode').val())},function(result){
		...
		switch(data.result){
			...
			case 'incorrect':alert('提示：该用户不存在，或用户名、密码输入错误！');break;
				 if(flag==0||setMsgValue()==0){
...
//				window.location.href="/examNappr/center/html/view/template/main.html?admin_id=" + 1 + "&priviledges = super";
				window.location.href="/examNappr/center/html/view/template/main.html";
				break;
			default:
