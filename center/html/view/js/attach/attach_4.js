...
//	alert("value="+value+"  name="+name);
	...
	if(value==1){
		// 通过
		$.post('/modules/php/action/page/projectapp/projectapp.act.php?action=execute&name=通州区支持承担市级以上科技项目申报书',function(result){
			if(result['code']!=0){
...
//获取参数
function getAttr(){
	var table_name = encodeURI("通州区支持承担市级以上科技项目申报书");
	$.post('/modules/php/action/page/attach/attach4.act.php?action=attach_attr&table_name='+table_name,function(result){
	    	var res = eval("("+result+")");
...
/*			if(i==1){
	...
	$('.org_info').bind('click',function(){
		$('.iframe iframe').attr('src','../../../../../website/html/view/template/apply/attach_4/org_info.html?status='+table_status);
	});
	
	$('.project_info').bind('click',function(){
		$('.iframe iframe').attr('src','../../../../../website/html/view/template/apply/attach_4/project_info.html?status='+table_status);
	});
	$('.centercheck').bind('click',function(){
		$('.iframe iframe').attr('src','/center/php/action/page/check_opinion.php?table_status='+table_status+ "&table_name='通州区支持承担市级以上科技项目申报书'"+ "&user_type=" + user_type);
	});
