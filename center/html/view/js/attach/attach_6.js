...
function execute(){
	$.post('../../../../../../modules/php/action/page/projectapp/projectapp.act.php?action=execute&name=通州区技术合同登记奖励资金申报表',function(result){
		if(result['code']!=0){
			alert("提交成功");
...
//获取参数
function getAttr(){
	var table_name = encodeURI('通州区技术合同登记奖励资金申报表');
...
/*			if(i==1){
	...
	$('.org_info').bind('click',function(){
		$('.iframe iframe').attr('src','../../../../../website/html/view/template/apply/attach_6/org_info.php?status='+table_status);
	});
	
	$('.check').bind('click',function(){
		$('.iframe iframe').attr('src',"../../../../../center/php/action/page/check_opinion.php?table_name='通州区技术合同登记奖励资金申报表'"+ "&user_type=" + user_type+"&status="+table_status);
	});
}
