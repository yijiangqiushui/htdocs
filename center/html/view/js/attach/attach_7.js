...
function execute(){
	$.post('../../../../../../modules/php/action/page/projectapp/projectapp.act.php?action=execute&name=通州区支持技术输出能力提升项目申报书',function(result){
		if(result['code']!=0){
			alert("提交成功");
...
//获取参数
function getAttr(){
	$.post('/modules/php/action/page/attach/attach7.act.php?action=attach_attr&table_name=通州区支持技术输出能力提升项目申报书',function(result){
	    	var res = eval("("+result+")");
...
/*			if(i==1){
	...
	$('.org_info').bind('click',function(){
		$('.iframe iframe').attr('src','../../../../../website/html/view/template/apply/attach_7/org_info.html?status='+table_status);
	});
	$('.check').bind('click',function(){
		$('.iframe iframe').attr('src',"../../../../../center/php/action/page/check_opinion.php?table_name='通州区支持技术输出能力提升项目申报书'" + "&user_type=" + user_type);
	});
}
