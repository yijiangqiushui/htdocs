...
//	alert("value="+value+"  name="+name);
	...
	if(value==1){
		// 通过
		$.post('/modules/php/action/page/projectapp/projectapp.act.php?action=execute&name=通州区支持产学研合作项目申请书',function(result){
			if(result['code']!=0){
...
//获取参数
function getAttr(){
	$.post('/modules/php/action/page/attach/attach2.act.php?action=attach_attr&table_name=通州区支持产学研合作项目申请书',function(result){
	    	var res = eval("("+result+")");
...
/*			if(i==1){
	...
	$('.org_info').bind('click',function(){
		$('.iframe iframe').attr('src','../../../../../website/html/view/template/apply/attach_2/org_info.html?status='+table_status);
	});
	
	$('.coorg_info').bind('click',function(){
		$('.iframe iframe').attr('src','../../../../../website/html/view/template/apply/attach_2/coorg_info.php?status='+table_status);
	});
	
	$('.project_info').bind('click',function(){
		$('.iframe iframe').attr('src','../../../../../website/html/view/template/apply/attach_2/project_info.php?status='+table_status);
	});
	
	$('.centercheck').bind('click',function(){
		$('.iframe iframe').attr('src','/center/php/action/page/check_opinion.php?table_status='+table_status+ "&table_name='通州区支持产学研合作项目申请书'"+ "&user_type=" + user_type );
	});
	$('.approve').bind('click',function(){
		$('.iframe iframe').attr('src','../../../../../website/html/view/template/Projectapp/project_opinion_promise.php?status='+table_status);
	});
}
