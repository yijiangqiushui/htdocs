...
function initModules(){
	...
	for(var i=0;i<MENUARR.length;i++){
		menu_html+='<a href="javascript:showSubmenu('+i+');" id="nav_'+i+'"'+(i==0?' style="color:#367abb; background-color:#e2e9ea; background-image:url();"':'')+'>'+MENUARR[i].menu_name+'</a>';
		...
		for(var j=0;j<MENUARR[i].submenu_arr.length;j++){
			submenu_html+='<div class="menuBlock"><h1>'+MENUARR[i].submenu_arr[j].submenu_name+'<span class="toggleSubmenu"></span></h1><ul class="submenuItem" style="margin-top:5px;">';
		  	var item_html='';
...
function showSubmenu(id){
	...
	//changeNav(MENUARR[id].menu_name, '/website/html/view/template/show.php');
	$('.menu .holder').css('display','none');
	$('#submenu_holder_'+id).css('display','block');
	
	$('.nav .left a').css({'color':'#fff','background-image':'url(../img/nav_bg.png)'});
	$('#nav_'+id).css({'color':'#367abb','background-color':'#e2e9ea','background-image':'url()'});
}
...
/*
 * 
 *这个使用来统一全局的project_id的值，将值放入Session中去
 */
