...
// 		echo $sql;
	...
	public function GetInfo_word($id, $name, $tid) {
	    $sql = "SELECT * FROM $name WHERE $tid = '$id' ";
	    //  		echo $sql;
	    $r = $this->select ( $sql );
	    return $r;
...
// 		echo $sql;
	...
	//LM
	public function GetInfo5($id, $name, $tid, $id1, $tid1) {
		$sql = "SELECT * FROM $name WHERE $tid = '$id' and $tid1 = '$id1'";
		$r = $this->select ( $sql );
		return $r;
	}
	...
	public function InsertDataIdentity($name, $data){
	    $field = implode ( ',', array_keys ( $data ) ); // 定义sql语句的字段部分
	    $i = 0;
	    $value = '';
	    foreach ( $data as $key => $val ) {
	    ...
	    $sql = "INSERT INTO $name ($field) VALUES ($value) SELECT @@IDENTITY AS 'identity' ";
	    return $this->insert ( $sql );
	}
...
// 			echo $sql;
	 ...
	 * 2015.09.29 Fred 写
	 */
	public function updateInfo5($name, $tid, $id, $data) { 
		// 进行查询
		$selSql = "select * from $name where $tid ='$id'";
		// 		echo $selSql;
		$result = $this->Select ( $selSql );
...
// 			echo $sql;
	 ...
	 * david 获取word写入数据
	 */
	function getDyData($name,$rid,$id){
	    $db = new DB();
	    $sql = "Select * from  $name  where  $rid = '$id'";
// 	    echo $sql;
...
// 	    var_dump($data);
	 ...
	 * david 获取word写入数据
	 */
	function getDyDataTwo($name,$rid,$id,$flag){
	    $db = new DB();
	    $sql = "Select * from  $name  where  $rid = '$id'  and  is_new = $flag " ;
// 	    echo $sql;
	    $result = $db->Select($sql);
// 	    var_dump($result);
// 	    exit();
...
// 	    var_dump($data);
	 ...
	 * lm 获取word写入数据
	 */
	function getDyData2($name,$rid,$id){
	    $db = new DB();
	    $sql = "Select * from  $name  where  $rid = '$id'";
// 	    echo $sql;
	    $result = $db->Select($sql);
// 	    
	...
	function getAppointField ($name,$rid,$id,$param){
		$db = new DB();
		$sql = "Select * from  $name  where  $rid = '$id'";
		// 	    echo $sql;
		$result = $db->Select($sql);
...
function DeleteAll($project_id,$table_name){
...
function DeleteBySub($table_name,$project_id,$num){
	$sql ="delete from $table_name where  project_id= '$project_id' and  subtable_id = '$num' ";
    $db=new DB();
    $db->Delete($sql);
    $db->Close();
}
